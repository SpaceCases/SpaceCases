...
def create_embed(msg_content: str, color: discord.Color = discord.Color.dark_theme()) -> discord.Embed:
...
async def send_success_embed(interaction: discord.Interaction, msg_content: str, ephemeral: bool = False) -> None:
...
async def send_err_embed(interaction: discord.Interaction, msg_content: str, ephemeral: bool = False) -> None:
...
async def yes_no_embed(
    ...
    await interaction.response.send_message(embed=embed, view=YesNoEmbedView(on_yes, on_no, interaction, timeout))


def get_rarity_embed_color(rarity: Rarity) -> int:
    return [0xB0C3D9, 0x5E98D9, 0x4B69FF, 0x8847FF, 0xD32CE6, 0xEB4B4B, 0xE4AE39][rarity.value]
