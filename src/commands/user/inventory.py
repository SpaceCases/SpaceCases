...
def get_inventory_item_list_string(bot: SpaceCasesBot, items: Iterable[tuple[str, int]]) -> str:
    item_strings = []
    for unformatted_name, count in items:
        item_metadata = bot.item_metadata[unformatted_name]
        if count == 1:
            item_strings.append(f"• {item_metadata.formatted_name} - **{currency_str_format(item_metadata.price)}**")
        else:
            item_strings.append(f"• {item_metadata.formatted_name} **X{count}** - **{currency_str_format(item_metadata.price * count)}** (**{currency_str_format(item_metadata.price)}** each)")
    return "\n".join(item_strings)
...
async def inventory(
    ...
    does_user_exist = (await bot.db.fetch_from_file("does_user_exist.sql", target_user.id))[0]["exists"]
    ...
    if item_name is None:
        skins_result = await bot.db.fetch_from_file("inventory/get_user_skins.sql", target_user.id)
        stickers_result = await bot.db.fetch_from_file("inventory/get_user_stickers.sql", target_user.id)
        ...
        if len(skins_result) > 0:
            e.add_field(
                name="Skins",
                value=get_inventory_item_list_string(bot, ((skin["name"], len(skin["floats"])) for skin in skins_result)),
                inline=False,
            )
        if len(stickers_result) > 0:
            e.add_field(
                ...
                value=get_inventory_item_list_string(
                    bot,
                    ((sticker["name"], sticker["count"]) for sticker in stickers_result),
                ),
                inline=False,
            )
        # determine inventory value
        skins_value = sum(bot.item_metadata[skin["name"]].price * len(skin["floats"]) for skin in skins_result)
        stickers_value = sum(bot.item_metadata[sticker["name"]].price * sticker["count"] for sticker in stickers_result)
        ...
        if isinstance(item_metadatum, SkinMetadatum):
            skin_result = await bot.db.fetch_from_file("inventory/get_skin.sql", target_user.id, unformatted_item_name)
        ...
        if isinstance(item_metadatum, StickerMetadatum):
            sticker_result = await bot.db.fetch_from_file("inventory/get_sticker.sql", target_user.id, unformatted_item_name)
            ...
            e.add_field(name="Rarity", value=item_metadatum.rarity.get_name_for_regular_item())
        e.set_footer(
...
async def item_name_autocomplete(bot: SpaceCasesBot, interaction: discord.Interaction, current: str) -> list[discord.app_commands.Choice]:
    try:
    ...
    for skin in skins_result:
        ...
        choices.append(discord.app_commands.Choice(name=choice_name, value=unformatted_name))
    # sticker choices
    stickers_result = await bot.db.fetch_from_file("inventory/get_user_stickers.sql", target_user.id)
