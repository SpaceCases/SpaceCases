...
claim_streak AS (
    SELECT 
        CASE 
            ...
            WHEN last_claim::date < CURRENT_DATE - INTERVAL '1 day' THEN 1  -- Reset streak if break more than 1 day
            ELSE claim_streak  -- Keep current streak if claim is made today
        END AS streak
    FROM "users"
    WHERE id = $1
),
claim_amount AS (
    SELECT LEAST(10000 + (claim_streak.streak - 1) * 2000, 30000) AS amount
    FROM claim_streak
),
updated_user AS (
    UPDATE "users"
    ...
    FROM claim_amount, claim_streak
    WHERE id = $1 AND last_claim::date < CURRENT_DATE
    RETURNING balance, claim_streak.streak, claim_amount.amount
)
SELECT 
    CASE 
